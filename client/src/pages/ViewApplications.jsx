    ...
    const changeJobApplicationStatus = async (id, status) => {
        try {
            ...
            if (data.success) {
                ...
                fetchCompanyJobApplications() // Refresh the applications list after updating status
            } else {
                toast.error(data.message)
            }
        } catch (error) {
                        ...
                        {applicants.filter(item => item.jobId && item.userId).map((applicant,index)=>(
                            <tr key={index} className='text-gray-700'>
                                ...
                                <td className='py-2 px-4 border-b border-gray-300 '>
                                    <a href={applicant.userId.resume} className='bg-blue-50 text-blue-400 px-3 py-1 cursor-pointer rounded gap-2 items-center inline-flex' target='_blank'>
